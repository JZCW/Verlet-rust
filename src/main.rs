...
fn main() -> Result<(), slint::PlatformError> {
  ...
  const OBJECT_SPAWN_POSITION:slover::Vector2f = slover::Vector2f{x:CONSTRAINT_RADIUS as f32,y:(CONSTRAINT_RADIUS as f32)*0.4};  // 新对象生成点
  const OBJECT_SPAWN_SPEED:slover::Vector2f = slover::Vector2f{x:1.,y:-1.}; // 对象初始速度
  // //const float        object_min_radius     = 1.0f;              // 对象尺寸最小值
  // //const float        object_max_radius     = 20.0f;             // 对象尺寸最大值
  ...
  s.constraint_center = slover::Vector2f{x:CONSTRAINT_RADIUS as f32 + 10. ,y:CONSTRAINT_RADIUS as f32};
  ...
  // s.addObject(slover::Vector2f{x:460.,y:80.},8.0,tiny_skia::Color::from_rgba8(255,0,255,127));
  // s.addObject(slover::Vector2f{x:440.,y:80.},8.0,tiny_skia::Color::from_rgba8(255,0,255,127));
  let ui_handle = ui.as_weak();
  let mut clock = std::time::Instant::now();
  let mut timer_gen_obj:u32 = 0;
  let mut color = renderer::rainbow_iter(0.9);
  ...
  frametimer.start(TimerMode::Repeated, std::time::Duration::from_millis(FRAME_TIME.into()), move || {
    ...
    if (rate>FRAME_MIN) && (num<MAX_OBJECTS_COUNT) && (timer_gen_obj>OBJECT_SPAWN_DELAY) {
      timer_gen_obj = 0;
      s.addObject(OBJECT_SPAWN_POSITION,OBJECT_RADIUS, OBJECT_SPAWN_SPEED, color.next().unwrap());
    }
    timer_gen_obj += 1;
